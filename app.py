...
# 3) Embeddings & FAISS (persist once)
...
# 4) A short, 2-sentence max prompt that paraphrases
STUFF_PROMPT = """You are Jun Le’s friendly assistant. Using the context below, answer the user in **at most two sentences**, and paraphrase rather than copy.
...
# 5) LLM + conversation chain w/ memory
llm = Bedrock(
    model_id=LLM_MODEL,
...
async def main(message: cl.Message):
    ...
    # 6a) Send a one-time “hello” before doing anything else
    if first_message:
        await cl.Message(
            content="👋 Hi, I’m Jun Le’s personal assistant! Ask me anything about Jun Le in two sentences or less."
        ).send()
        first_message = False

    # 6b) “I don’t know” fallback
